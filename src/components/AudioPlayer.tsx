...
export default function AudioPlayer() {
  ...
  // Synchronized refs to allow seamless mount event handling without stale closures
  ...
  const handleNextTrackRef = () => {
    const currentTracks = tracksRef.current;
    if (currentTracks.length === 0) return;
    const nextIndex = (currentTrackIndexRef.current + 1) % currentTracks.length;
    setCurrentTrackIndex(nextIndex);
    setIsPlaying(true);
    localStorage.setItem("bg_music_playing", "true");
  };

  // Create YouTube Player instance strictly ONCE
  useEffect(() => {
    ...
    const initPlayer = () => {
      ...
      const activeVideoId = currentTracks[index]?.id || "";
          ...
          events: {
            onReady: (event: any) => {
              ...
              try {
                ...
                console.warn("YouTube inside ready handler failed: " + (e instanceof Error ? e.message : String(e)));
              }
            },
            ...
            onError: (err: any) => {
              console.warn("YouTube player video warning (skipping track): " + (err?.data !== undefined ? err.data : String(err)));
              setTimeout(() => {
                handleNextTrackRef();
              }, 1200);
            }
    ...
    return () => {
      if (intervalId) clearInterval(intervalId);
      // Wait, we do NOT call destroy() here to prevent deleting iframe node from root DOM!
    };
  }, []); // Run strictly once on mount

  // React to track index changes dynamically without recreate
  useEffect(() => {
    if (playerRef.current && playerReady) {
      ...
      if (activeVideoId) {
        try {
          ...
          const loadedVideoId = yt.getVideoData ? yt.getVideoData().video_id : null;
          if (loadedVideoId !== activeVideoId) {
            yt.loadVideoById ? yt.loadVideoById(activeVideoId) : null;
            setIsPlaying(true);
          }
        } catch (e) {
          console.warn("Failed switching video source inside load track module: " + (e instanceof Error ? e.message : String(e)));
        }
      }
    }
  }, [currentTrackIndex, tracks, playerReady]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Pause state synchronizer error: " + (e instanceof Error ? e.message : String(e)));
    }
  ...
  useEffect(() => {
    const handleFirstUserInteraction = () => {
      const savedPlayState = localStorage.getItem("bg_music_playing");
      // Autoplay is enabled by default unless the visitor explicitly paused it previously
      if (savedPlayState !== "false" && !isPlaying) {
        setIsPlaying(true);
        setShowTooltip(false);
        localStorage.setItem("bg_music_playing", "true");
        // Force trigger play on player
        const yt = playerRef.current;
  ...
  return (
    <div className="fixed bottom-6 left-6 z-50 flex flex-col items-start gap-3 select-none">
      ...
      {/* Technically visible but visually negligible YouTube Player container to bypass all Chrome, iOS & Safari energy saving / autoplay blocks */}
      <div 
        className="fixed bottom-2 left-2 pointer-events-none z-[-50] overflow-hidden select-none"
        ...
        {isHidden ? (
          ...
          >
            <motion.button
              ...
              className="p-3.5 rounded-full bg-black/90 border border-[#D4AF37]/50 text-[#D4AF37] hover:bg-neutral-900 transition-all shadow-2xl flex items-center justify-center hover:shadow-[#D4AF37]/20 relative group cursor-pointer"
